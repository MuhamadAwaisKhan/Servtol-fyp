...
class _servicetypeState extends State<servicetype> {
  ...
  void _deleteServiceType(String id) async {
    try {
      // Step 1: Check for dependencies in related collections (e.g., 'Services')
      final QuerySnapshot dependencies = await _db
          .collection('Services') // Change this to the relevant collection name
          .where('ServiceTypeID', isEqualTo: id)
          .get();

      if (dependencies.docs.isNotEmpty) {
        // Step 2: Show a message if dependencies exist
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Cannot delete. This service type is linked to existing services.',
      ...
      // Step 3: Proceed with deletion if no dependencies are found
      showDialog(
        context: context,
              ...
              TextButton(
                ...
                onPressed: () async {
                  try {
                    ...
                    await _db.collection('ServiceTypes').doc(id).delete();

                    // Step 5: Notify the user of successful deletion
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
      ...
      // Catch any errors during the dependency check
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Error: Unable to check dependencies. $e',
            style: TextStyle(fontFamily: 'Poppins'),
          ),
          backgroundColor: Colors.redAccent,
        ),
